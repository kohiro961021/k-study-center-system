...
export function DatePicker({ value, onChange }: DatePickerProps) {
    ...
    // 3. Padding from next month to make grid a multiple of 7 (usually 42 cells total)
    const totalCells = 42;
    ...
    return (
        <div className="relative inline-block text-left" ref={containerRef}>
            ...
            <button
                ...
                className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-base bg-card hover:bg-slate-50 dark:hover:bg-slate-200/50 cursor-pointer transition text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-accent/50 select-none font-medium w-full md:w-auto justify-between md:justify-start"
            >
            ...
            {/* Calendar Popover */}
            {isOpen && (
                <div className="absolute left-1/2 -translate-x-1/2 md:translate-x-0 md:left-0 mt-2 w-80 bg-card/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl z-50 p-5 select-none animate-in fade-in slide-in-from-top-2 duration-200">
                    {/* Header */}
